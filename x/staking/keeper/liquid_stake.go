...
func (k Keeper) CheckExceedsGlobalLiquidStakingCap(ctx sdk.Context, tokens math.Int, sharesAlreadyBonded bool) (bool, error) {
...
func (k Keeper) CheckExceedsValidatorBondCap(ctx sdk.Context, validator types.Validator, shares math.LegacyDec) (bool, error) {
...
// Returns true if the cap is exceeded
func (k Keeper) CheckExceedsValidatorLiquidStakingCap(ctx sdk.Context, validator types.Validator, shares math.LegacyDec, sharesAlreadyBonded bool) (bool, error) {
...
func (k Keeper) SafelyIncreaseTotalLiquidStakedTokens(ctx sdk.Context, amount math.Int, sharesAlreadyBonded bool) error {
	exceeds, err := k.CheckExceedsGlobalLiquidStakingCap(ctx, amount, sharesAlreadyBonded)
	if err != nil {
		return err
	}
	if exceeds {
...
func (k Keeper) SafelyIncreaseValidatorLiquidShares(ctx sdk.Context, valAddress sdk.ValAddress, shares math.LegacyDec, sharesAlreadyBonded bool) (types.Validator, error) {
	...
	if err != nil {
		return validator, types.ErrNoValidatorFound.Wrapf("validator %s not found", valAddress)
	}

	// Confirm the validator bond factor and validator liquid staking cap will not be exceeded
	exceeds, err := k.CheckExceedsValidatorBondCap(ctx, validator, shares)
	if err != nil {
	...
	exceeds, err = k.CheckExceedsValidatorLiquidStakingCap(ctx, validator, shares, sharesAlreadyBonded)
	if err != nil {
		return validator, err
	}
	if exceeds {
...
func (k Keeper) IncreaseValidatorBondShares(ctx sdk.Context, valAddress sdk.ValAddress, shares math.LegacyDec) error {
	validator, err := k.GetValidator(ctx, valAddress)
	if err != nil {
		return types.ErrNoValidatorFound.Wrapf("validator %s not found", valAddress)
	}
...
func (k Keeper) GetAllTokenizeShareLocks(ctx sdk.Context) (tokenizeShareLocks []types.TokenizeShareLock) {
	...
	iterator := storetypes.KVStorePrefixIterator(runtime.KVStoreAdapter(store), types.TokenizeSharesLockPrefix)
...
func (k Keeper) RefreshTotalLiquidStaked(ctx sdk.Context) error {
	...
	for _, delegation := range allDelegations {
		...
		if k.DelegatorIsLiquidStaker(delegatorAddress) {
			...
			if err != nil {
				return types.ErrNoValidatorFound.Wrapf("validator %s not found", validatorAddress)
			}
