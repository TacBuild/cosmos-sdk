...
func (k Keeper) AddTokenizeShareRecord(ctx context.Context, tokenizeShareRecord types.TokenizeShareRecord) error {
	if k.hasTokenizeShareRecord(ctx, tokenizeShareRecord.Id) {
		return types.ErrTokenizeShareRecordAlreadyExists.Wrapf("TokenizeShareRecord already exists: %d", tokenizeShareRecord.Id)
	}
...
func (k Keeper) hasTokenizeShareRecord(ctx context.Context, id uint64) bool {
	store := k.storeService.OpenKVStore(ctx)
	res, err := store.Has(types.GetTokenizeShareRecordByIndexKey(id))
	if err != nil {
		return false
	}
	return res
